...
//                L move    R move  speed
int32_t Steps[] = { 125,     125,     8,
                      0,       0,    -1 };

int32_t cenTurn[] = {-360, 360, 10,
                     0, 0, -1};

int32_t rightTurn[] = {-180, 180, 10,
                     0, 0, -1};

int32_t leftTurn[] = {180, -180, 10,
                     0, 0, -1};
...
void TA1_0_IRQHandler(void) {
    ...
    switch (interval_i) {
        case 0: // LED1 on
            ...
            break;
        case 1: // sample ADC
            ADC_In17_21_23(&v17, &data2, &data1);
            break;
        case 2: // LED1 off
            P6->OUT &= ~BIT2;
            break;
        case 3: // LED2 on
            ...
            break;
        case 4: // sample ADC
            ADC_In17_21_23(&data3, &data2, &v23);
            break;
        case 5: // LED2 off
            P10->OUT &= ~BIT0;
            break;
        case 6: // LED3 on
            ...
            break;
        case 7: // sample ADC
            ADC_In17_21_23(&data3, &v21, &data1);
            break;
        case 8: // LED3 off
            P10->OUT &= ~BIT2;
    }
...
int main(void) {
    ...
    while (1) {
        ...
        if (tick_now != last_tick && !Stopped) {
            ...
            if ((close(N, Finish, NCH) || 0==Step_speed) && (turn == 0||turning ==1)) {
                Step_i++;
                if (Steps[Step_i*3 + 2] < 0) {
                    Step_i = 0;
                }
            ...
            else if(close(N, Finish, NCH) || 0==Step_speed || turn == 1){
                Step_j++;
                if (cenTurn[Step_j*3 + 2] < 0) {
                    Step_j = 0;
                    turn = 0;
                }

                Step_speed = cenTurn[Step_j*3 + 2];
                Move[0] = cenTurn[Step_j*3 + 0]; // left
                Move[1] = cenTurn[Step_j*3 + 1]; // right
                Start[0] = N[0];
                Start[1] = N[1];
                Setpoint[0] = N[0];
                Setpoint[1] = N[1];
            ...
            else if(close(N, Finish, NCH) || 0==Step_speed || turn == 2){
                Step_k++;
                if (rightTurn[Step_k*3 + 2] < 0) {
                    Step_k = 0;
                    turn = 0;
                }

                Step_speed = rightTurn[Step_k*3 + 2];
                Move[0] = rightTurn[Step_k*3 + 0]; // left
                Move[1] = rightTurn[Step_k*3 + 1]; // right
                Start[0] = N[0];
                Start[1] = N[1];
                Setpoint[0] = N[0];
                Setpoint[1] = N[1];
            ...
            else if(close(N, Finish, NCH) || 0==Step_speed || turn == 3){
                Step_l++;
                if (leftTurn[Step_l*3 + 2] < 0) {
                    Step_l = 0;
                    turn = 0;
                }

                Step_speed = leftTurn[Step_l*3 + 2];
                Move[0] = leftTurn[Step_l*3 + 0]; // left
                Move[1] = leftTurn[Step_l*3 + 1]; // right
                Start[0] = N[0];
                Start[1] = N[1];
                Setpoint[0] = N[0];
                Setpoint[1] = N[1];
